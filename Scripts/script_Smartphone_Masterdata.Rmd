...
# cameras <- product_html %>% xml_find_all("//tr//td[@data-spec = 'cam1modules']") %>% xml_text()
# camera1 <- camera1 %>% str_extract(".*(?=\\r\\n)")
# cameras <- paste0(cameras, "\r\n")
# cameras %>% str_extract_all("(?=\r\n).*?(?=\r\n)")
```


```{r}
# battery_charge_raw <- battery_charge_raw[2]
# battery_charge_raw <- battery_charge_raw %>% xml_text()
# battery_charge <- battery_charge_raw %>% str_extract(".*") %>% str_remove_all("\\(advertised\\)") %>% str_squish()
# battery_charge
# battery_wireless_raw <- battery_charge_raw %>% str_remove_all("/r/") %>% str_squish() %>% toupper()
# if (grepl("WIRELESS", battery_wireless_raw)){
...
view(df_md %>% select(NAME, BATTERY_TYPE_EXTENDED, BATTERY_TYPE))
```
...
```{r}
write_csv2(df_md, "C:/Users/Jorge Pascual S/Desktop/Web-scraping-Smartphones-prices-in-Spain/Data/DF_MD.csv")
```